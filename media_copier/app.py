...
from .copier import copy_import_plan
...
class MediaDateCopierApp:
    def __init__(self, root: Tk) -> None:
        ...
        self.current_file_var = StringVar(value="等待预览")
        self.status_var = StringVar(value="请选择源文件夹和目标文件夹，然后点击预览导入")
        self.stats_var = StringVar(value="成功 0 / 跳过 0 / 改名 0 / 失败 0")
        self.speed_var = StringVar(value="当前速度 0 B/s / 平均 0 B/s / 已处理 0 B")
        self.preview_summary_var = StringVar(value="尚未预览")
    ...
    def _build_ui(self) -> None:
        ...
        Entry(paths_frame, textvariable=self.source_var).grid(row=0, column=1, sticky="ew", padx=8, pady=8)
        Button(paths_frame, text="选择源", command=self.choose_source).grid(row=0, column=2, padx=10, pady=8)
        ...
        Checkbutton(types_frame, text="照片", variable=self.photos_var, command=self._on_mode_changed).grid(row=0, column=0, padx=10, pady=8, sticky="w")
        Checkbutton(types_frame, text="视频", variable=self.videos_var, command=self._on_mode_changed).grid(row=0, column=1, padx=10, pady=8, sticky="w")
        Checkbutton(types_frame, text="全部文件", variable=self.all_files_var, command=self._on_mode_changed).grid(row=0, column=2, padx=10, pady=8, sticky="w")
        ...
        self.preview_button.grid(row=0, column=0, padx=10, pady=8)
        Label(preview_frame, textvariable=self.preview_summary_var, anchor="w", justify="left").grid(
        ...
        self.start_button = Button(controls_frame, text="开始复制", command=self.start_copy, state="disabled")
        self.start_button.grid(row=0, column=0, padx=(0, 8), pady=4)
        self.cancel_button = Button(controls_frame, text="取消", command=self.cancel_copy, state="disabled")
    ...
    def choose_target(self) -> None:
        selected = filedialog.askdirectory(title="选择目标文件夹", initialdir=self.target_var.get().strip() or None)
        if selected:
    ...
    def start_preview(self) -> None:
        ...
        if settings is None:
            return
        source_dir, target_dir, extensions, template = settings

        self._save_current_config()
        self.import_plan = None
        self.start_button.configure(state="disabled")
        self.preview_button.configure(state="disabled")
        self.status_var.set("正在预览导入...")
        self.preview_summary_var.set("正在扫描文件并分析重复项...")
        self.current_file_var.set("正在预览导入...")
        self._clear_log()

        self.preview_worker = threading.Thread(
            target=self._preview_worker,
            args=(source_dir, target_dir, extensions, template),
            daemon=True,
        )
    ...
    def _preview_worker(
        ...
        try:
            ...
            self.events.put(("preview_error", f"预览失败：{exc}"))
            return
        self.events.put(("preview_done", plan))

    def start_copy(self) -> None:
        if self.import_plan is None:
            messagebox.showerror("无法开始", "请先点击“预览导入”。")
            return
        if self.import_plan.summary.total_files == 0:
            messagebox.showerror("无法开始", "没有可处理的文件。")
            return

        self.cancel_event.clear()
        self.progress.configure(maximum=max(self.import_plan.summary.total_bytes, 1), value=0)
        self.current_file_var.set("准备复制...")
        self.status_var.set("正在运行")
        self.stats_var.set("成功 0 / 跳过 0 / 改名 0 / 失败 0")
        self.speed_var.set("当前速度 0 B/s / 平均 0 B/s / 已处理 0 B")
        self.start_button.configure(state="disabled")
        self.preview_button.configure(state="disabled")
        self.cancel_button.configure(state="normal")

        self.worker = threading.Thread(
            target=copy_import_plan,
            args=(self.import_plan, self.cancel_event, self.events),
            daemon=True,
        )
        self.worker.start()
    ...
    def _validated_settings(self) -> tuple[Path, Path, set[str] | None, str] | None:
        ...
        return source_dir, target_dir, extensions, template
        ...
        save_config(
            {
                ...
                "duplicate_policy": "same_size_skip_else_rename",
                "verify_mode": "sha256",
            }
        )
    ...
    def _handle_event(self, event: tuple) -> None:
        ...
        if event_type == "preview_done":
            ...
            self._finish_preview_error(event[1])
            ...
            _, file_name, stats = event
            self.current_file_var.set(f"正在校验：{file_name}")
            self._update_progress(stats)
    ...
    def _finish_preview(self, plan: ImportPlan) -> None:
        self.import_plan = plan
        self.preview_button.configure(state="normal")
        self.start_button.configure(state="normal" if plan.summary.total_files > 0 else "disabled")
        self.status_var.set("预览完成")
        self.current_file_var.set("预览完成，确认后可开始复制")
        self.preview_summary_var.set(self._format_preview_summary(plan))
        self._append_log(self._format_preview_summary(plan).replace("\n", "；"))

    def _finish_preview_error(self, message: str) -> None:
        self.import_plan = None
        self.preview_button.configure(state="normal")
        self.start_button.configure(state="disabled")
    ...
    def _format_preview_summary(self, plan: ImportPlan) -> str:
        summary = plan.summary
        return (
            f"总数 {summary.total_files} / 总大小 {format_bytes(summary.total_bytes)} / "
            f"将复制 {summary.copy_count} / 跳过 {summary.skip_count} / "
            f"自动改名 {summary.renamed_count} / 错误 {summary.error_count} / "
    ...
    def _update_progress(self, stats: CopyStats) -> None:
        ...
        self.stats_var.set(
            f"成功 {stats.copied} / 跳过 {stats.skipped} / 改名 {stats.renamed} / "
            f"失败 {stats.failed} / 文件 {stats.processed_files}/{stats.total_files}"
        )
        self.speed_var.set(
    ...
    def _finish_copy(self, stats: CopyStats) -> None:
        self.start_button.configure(state="normal" if self.import_plan else "disabled")
        self.preview_button.configure(state="normal")
        self.cancel_button.configure(state="disabled")
        self._update_progress(stats)

        title = "已取消" if stats.cancelled else "复制完成"
        ...
        summary = (
            ...
            f"已复制：{stats.copied}\n"
            f"已跳过：{stats.skipped}\n"
            f"自动改名：{stats.renamed}\n"
            f"失败：{stats.failed}\n"
            f"校验成功：{stats.verify_success}\n"
            f"校验失败：{stats.verify_failed}\n"
